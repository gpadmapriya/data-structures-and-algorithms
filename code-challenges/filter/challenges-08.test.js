...
const filterStringsWithVowels = (arr) => {
  return arr.filter(word => {
    if (word.match(/[aeiou]/ig)) { return true; }
    return false;
  });
};
...
const getStatName = (arr, minBaseStat) => {
  let returnArray = arr.filter(statObj => { return statObj.baseStat > minBaseStat; });
  return returnArray.map(obj => obj.stat.name);
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
