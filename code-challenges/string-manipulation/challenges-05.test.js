...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    result.push(recipe.ingredients[i].slice(index + 1, recipe.ingredients[i].length));
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  for (var i = 0; i < recipe.ingredients.length; i++) {
    newArray.push(recipe.ingredients[i].split(' '));
  }

  for (i = 0; i < newArray.length; i++) {
    let str = newArray[i][2];
    for (let j = 3; j < newArray[i].length; j++) {
      if (newArray[i][j]) {
        str = str + ' ' + newArray[i][j];
      }
    }
    result.push(str);
  }
...
const stepActions = (recipe) => {
  ...
  for (var i = 0; i < recipe.steps.length; i++) {
    index = recipe.steps[i].indexOf(' ');
    str = recipe.steps[i].substring(0, index);
    result.push(str);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters > str.length) {
    ...
    index = str.length - numberOfCharacters;
