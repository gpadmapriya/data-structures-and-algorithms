...
const count = (target, input) => {
  /*let single = input.reduce((a, b) => {
    return a.concat(b);
  }, []);

  let newArr = single.filter((val) => {
    if (val === target) {
      return true;
    }
  });
  ...
  //Reshape the input array of arrays using reduce to a single concatenated array. 
  //Filter for the target value and return the length of the resulting array for the count.
...
let findMaleAndFemale = (data) => {
  return data.filter(person => {
    ...
    //return acc + person.name + ' and ';
    if (acc === '') {
      acc = acc + person.name;
    } else {
      acc = acc + ' and ' + person.name;
    }
    return acc;
  }, '');
};
